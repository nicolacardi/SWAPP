...
//INIZIO PAGINA VOTAZIONI **************************************************************************************************************
	...
	while (mysqli_stmt_fetch($stmt05)) {
		...
		//se voglioo includere anche i voti dati dai tutor dvo escludere dal filtro ID_mae_cov...in questo modo
		//seleziono per classe e non per ID_mae_cov
		//però così facendo arrivano anche altre MATERIE che non hanno a che fare col maestro corrente
		//bisognerebbe selezionare solo le materie sue
		// $sql1 = "SELECT DISTINCT ID_cov, codmat_cov, tipo_cov, data_cov, argomento_cov, descmateria_mtt, codmat_mtt FROM tab_compitiverifiche LEFT JOIN tab_materie ON codmat_mtt =  codmat_cov WHERE classe_cov = ? AND sezione_cov = ? AND annoscolastico_cov = ? ORDER BY codmat_cov, tipo_cov, data_cov";


		$sql1 = "SELECT DISTINCT ID_cov, codmat_cov, tipo_cov, data_cov, argomento_cov, descmateria_mtt, codmat_mtt FROM tab_compitiverifiche LEFT JOIN tab_materie ON codmat_mtt =  codmat_cov WHERE classe_cov = ? AND sezione_cov = ? AND annoscolastico_cov = ? AND codmat_mtt IN (SELECT codmat_cma FROM tab_classimaestri WHERE ID_mae_cma = ? AND classe_cma = ? AND sezione_cma = ? AND annoscolastico_cma = ? ) ORDER BY codmat_cov, tipo_cov, data_cov";

		$stmt1 = mysqli_prepare($mysqli, $sql1);
		//mysqli_stmt_bind_param($stmt1, "sssi", $classe_cla, $sezione_cla, $annoscolastico_cla, $ID_mae);
		//mysqli_stmt_bind_param($stmt1, "sss", $classe_cla, $sezione_cla, $annoscolastico_cla);
		mysqli_stmt_bind_param($stmt1, "sssisss", $classe_cla, $sezione_cla, $annoscolastico_cla, $ID_mae, $classe_cla, $sezione_cla, $annoscolastico_cla);
		...
		for ($pagina = 1; $pagina<=$pagine; $pagina++) {
			...
			while (mysqli_stmt_fetch($stmt2)) {
				...
				if (strlen( $nome_alu." ".$cognome_alu) > 21) {				//nome e cognome
					$pdf->Cell($cellW, $cellH, substr(utf8_decode($nome_alu)." ".utf8_decode($cognome_alu), 0, 20)."..." ,0,0,"L");
				} else {
					$pdf->Cell($cellW, $cellH, utf8_decode($nome_alu)." ".utf8_decode($cognome_alu) ,0,0,"L");
				}
				$alunni++;
			}
			$pdf->SetFont($fontdefault,'',9);
		...
		if ($mostragiudizi == 1) {
			...
			while (mysqli_stmt_fetch($stmt1)) {
				...
				$pdf->Cell($cellWGiu,$cellHArg,utf8_decode($argomento_cov)." del ".timestamp_to_ggmmaaaa($data_cov), 1 ,0, 'C');	//argomento
				$pdf->SetFont($fontdefault,'',10);
				$startY = 2;
				...
				while (mysqli_stmt_fetch($stmt2)) {
					...
					if (strlen( utf8_decode($nome_alu)." ".utf8_decode($cognome_alu)) > 21) {									//nome e cognome
						$pdf->Cell($col2, $cellHGiu, substr($nome_alu." ".$cognome_alu, 0, 20)."..." ,0,0,"L");
					} else {
						$pdf->Cell($col2, $cellHGiu, utf8_decode($nome_alu)." ".utf8_decode($cognome_alu) ,0,0,"L");
