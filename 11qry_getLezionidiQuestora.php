...
?>
	<select id="selectLezionidiQuestOra" style="margin-left:0px;" onchange="updateBtnOK2();">
		...
		<?;
		$sql = "SELECT ID_ora, ID_mae_ora, classe_ora, sezione_ora, data_ora, ora_ora, codmat_ora, descmateria_mtt, nome_mae, cognome_mae FROM (tab_orario LEFT JOIN tab_materie ON codmat_ora = codmat_mtt LEFT JOIN tab_anagraficamaestri ON ID_mae = ID_mae_ora) LEFT JOIN tab_classi ON classe_ora = classe_cls WHERE data_ora = ? AND ora_ora = ? AND codmat_ora <> 'XX1' AND codmat_ora <> 'XX3' AND codmat_ora <> 'XX2' AND codmat_ora <> 'XX4' AND codmat_ora <> 'XX5' AND codmat_ora <> 'nom' AND firma_mae_ora = 0 ORDER BY ord_cls";
