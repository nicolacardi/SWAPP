...
<script>
	...
	function salvaVoti () {
		$('.voto_vcc').each(function(){
			let idvoto_vcc = $(this).attr('id');
			let n = idvoto_vcc.indexOf(".");
			let ID_alu_vcc = parseInt(idvoto_vcc.substr(4,n-4));
			let ID_cov_vcc = parseInt(idvoto_vcc.substr(n+1));
			let voto_vcc = $(this).val();
			//>>>>>>>>>>>>>>>>>>>>>> a questo punto posso salvare. Opzione 1: cancello tutto e salvo...ma "cosa" cancello? opzione2 scrivo la SQL del tipo IF EXISTS allora update altrimenti INSERT. SOLO SE voto <> ""? eh no, perchè se uno ha cancellato? dovrei fare che se è "" allora faccio una DELETE...
