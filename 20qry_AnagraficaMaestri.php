<?include_once("database/databaseii.php");
	...
	if (isset ($_POST['ord4'])){
		$ord4 = $_POST['ord4'];
		$ordsql = orderbysql( $ord4, 'in_organico_soc', $ordsql);
	}
	...
	$sql = "SELECT DISTINCT ID_soc, nome_soc, cognome_soc, tipo_soc, dataiscrizione_soc FROM tab_anagraficamaestri ".$whereannocorrente.$ordsql;
	//DA RENDERE PARAMETRICA - DIFFICILE
	$stmt = mysqli_prepare($mysqli, $sql);
	mysqli_stmt_execute($stmt);
	mysqli_stmt_bind_result($stmt, $ID_soc, $nome_soc, $cognome_soc, $tipo_soc, $dataiscrizione_soc);
	$riga =  0;
	...
	$riga++;	?>
		<tr>
			...
			<td>
				<input class="tablecell6 disab val<?=$ID_soc?>" style="width: 150px;" type="text"  id="nome_soc<?=$ID_soc?>" name="nome_soc" value = "<?=$nome_soc?>" disabled>
			</td>
			<td>
				<input class="tablecell6 disab val<?=$ID_soc?>" style="width: 150px;" type="text" id="cognome_soc<?=$ID_soc?>" name="cognome_soc" value = "<?=$cognome_soc?>" disabled>
			</td>
			<td>
				<input class="tablecell6 disab val<?=$ID_soc?> w100px" type="text" id="tipo_soc<?=$ID_soc?>" name="tipo_psoc" value = "<?=$tiposocA[$tipo_soc]?>" disabled>
			</td>
		</tr>
	<?}?>
<script>
