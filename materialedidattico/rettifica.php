...
if (loggedin()) {
	...
	if (isset($_GET['art_id'])){
        ...
        $query = "SELECT `ARTFOR_CODEXT`, `ARTFOR_PXTOT`, `ARTFOR_CONF`, `ARTFOR_PXUNIT`, `ARTFOR_GG_REORDER` FROM `articolifornitori` WHERE `ART_ID`='".$art_id."' AND `ARTFOR_DEFAULT`= '1'"; 
        $stmt = mysqli_prepare($mysqli, $query);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_bind_result($stmt, $artfor_codext, $artfor_pxtot, $artfor_conf, $artfor_pxunit, $artfor_gg_reorder);
        mysqli_stmt_store_result($stmt);
        if (mysqli_stmt_num_rows($stmt) ==1) {
            while (mysqli_stmt_fetch($stmt)) {	
			} //fine while
	...
	if(isset($_POST['salva'])) {
		...
		if (isset($_POST['ART_QTY_MOD'])) { //significa: "se è stato premuto il tasto submit"
			//assegna le variabili
			$art_qty_mod = $_POST['ART_QTY_MOD'];
			if (!empty ($art_qty_mod)) {
							
							
							$newart_qty_stock = $art_qty_stock + $art_qty_mod;
							
							if ($newart_qty_stock >=0) {
							
								$query2 = "UPDATE `articoli` SET `ART_QTY_STOCK` = '".$newart_qty_stock."' WHERE `ART_ID` = '".$art_id."'";
                                $stmt2 = mysqli_prepare($mysqli, $query2);
                                mysqli_stmt_execute($stmt2);
								
								//Ora aggiorna la tabella s_carichi
								$query3 ="INSERT INTO `s_carichi` ( `S_CA_TYPE`, `ART_ID`, `FOR_ID`, `S_CA_QTY_UNIT_S_CARICO`, `S_CA_PXUNIT`, `S_CA_PXTOT` ) VALUES ( 'rettifica', '".$art_id."', '".$for_id."', '".$art_qty_mod."', '".$artfor_pxunit."', '".($art_qty_mod * $artfor_pxunit)."'  )";
								...
								if ($logsino == 1) {
									$query4 ="INSERT INTO `log` ( `USER_ID`, `OPERAZIONE`, `QUANTITA`, `ART_ID`, `ART_DESC`, `FOR_ID`, `FOR_RAGSOC`, `PXTOT` ) VALUES ( '".$_SESSION['user_id']."', 'rettifica', '".$art_qty_mod."', '".$art_id."', '".$art_desc."', '".$for_id."', '".$for_ragsoc."', '".($art_qty_mod * $artfor_pxunit)."'  )";
								...
								echo '<h3 class="info"> Rettificate le quantità a magazzino di '.$art_qty_mod.' unità</h3>';
								$art_qty_mod = 0;
							} else {
								echo '<h3 class="warn"> Rettifica non effettuabile in quanto questo determinerebbe quantità negative a stock per : '.$art_desc.'</h3>';
							}
						} else {
		
							echo '<h3 class="info"> Inserire un valore diverso da 0</h3>';
						} //fine if (!empty($art_qty_mod))
		
			
		} else {
			echo '<h3 class="warn"> not all set</h3>';
		
		} //fine if (isset($_POST['ART_QTY_MOD']))
...
<div class="moduli">
    <form action="rettifica.php" method="POST">
    <input type="hidden" name="ART_ID" value="<?php if (isset($art_id)){echo $art_id;} ?>"></div>
        ...
        <tr style="background-color:white;">
            <td colspan="8">
            <h3>- RETTIFICA QUANTITA' a MAGAZZINO ARTICOLO -</h3><br />
            </td>
        </tr>
        ...
        <tr>
            ...
            <td colspan="2">
                <h3>Rettifica (+/-)<br />quantità a Magazzino</h3>
            </td>
			<td colspan="2">
        ...
        <tr>
            <td colspan="2">
                	<h5><?php
                    $query5 = "SELECT `TIP_DESC` FROM `tipologie` WHERE `TIP_ID` = '".$tip_id."'";
                    $stmt5 = mysqli_prepare($mysqli, $query5);
                    mysqli_stmt_execute($stmt5);
                    mysqli_stmt_bind_result($stmt5, $tip_desc);
            ...
            <td colspan="2">
            	<input class="evident" id="qtytomodify" type="textarticolo" maxlength="6" name="ART_QTY_MOD" value="<?php if (isset ($art_qty_mod)){echo $art_qty_mod ;} ?>">		
            </td>
            <td colspan="2">
...
<!-- e poi invece la INPUT quantità richiesta di confezioni, il numero di unità che questo significa per la scuola e il prezzo totale -->
    	...
    	<tr>
        	...
        	<td colspan="2">
        		<br><br> <input name="salva" id="submit3" type="submit" style="margin: auto;" value="Lancia Rettifica" Onclick="return confirm ('ATTENZIONE: OPERAZIONE DI RETTIFICA IRREVERSIBILE')">
        	</td>
        	<td colspan="3">
            </td>
        </tr>
