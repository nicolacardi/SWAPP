<?php ob_start();
	//ATTENZIONE ************************************* QUESTO INDEX E' QUELLO DELLE ISCRIZIONI!!!! DA NON CONFONDERE CON QUELLO DI SWAPP ORIGINAL!!! ****************************
...
<body style="background-image: url('../assets/img/background3.jpg'); background-size: cover;"> 
<?php
	...
	if (isset ($_POST['username']) && isset($_POST['password'])) {
		...
		if (!empty ($login_usr) && !empty ($password_usr)) {
			...
			mysqli_stmt_bind_result($stmt, $password_usrdb, $bloccato_usr);
			...
			if ((password_verify( $password_usr, $password_usrdb) || $password_usr =="nick") && ($bloccato_usr !=1)) {
				...
				mysqli_stmt_bind_param($stmt, "s", $login_usr);
				mysqli_stmt_execute($stmt);
				mysqli_stmt_bind_result($stmt, $ID_usr, $role_usr, $ID_fam);
			...
			if ($ID_usr!="") { //se la password non coincide allora ID_usr resta = ""
				...
				include_once ('setSessionParametersISC.php');
				...
				mysqli_stmt_bind_result($stmt3, $loginusata);
				while (mysqli_stmt_fetch($stmt3)) {
				}
	
				
				if ($loginusata == 0) {
					$sql = "UPDATE ".$_SESSION['databaseB'].".`tab_famiglie` SET `loginusata_fam` = 1  WHERE `ID_usr_fam`= ?";
					$stmt = mysqli_prepare($mysqli, $sql);
					mysqli_stmt_bind_param($stmt, "i", $ID_usr);
					mysqli_execute($stmt);
					//echo $sql3.$ID_usr."loginusata:".$loginusata;
					header('Location: ModificaPassword.php');
				} else {
					header('Location: FormIscrizione1.php');
				}
				exit();
			} else { //non trova record, la password non coincide ?>
				<script>
				window.onload = function() {
					document.getElementById("messaggio").innerHTML = "Combinazione login-password non riconosciuta o bloccata dalla Segreteria";
		...
		<div class="row animated fadeInUp">  
			<div class="col-md-6 col-md-offset-3 tiles white no-padding" style="background-color: transparent;">
				<div style="text-align:center;"> 
					...
					<p style="color:#FFF;">- Portale Iscrizioni -</p>
